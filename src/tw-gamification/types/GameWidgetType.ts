...
export abstract class GameWidget extends Widget {
   ...
   * @return handled - return true if the event is handled, and we will safely remove handled events from the cache; false if not handled, we will keep the cache untouched for next try. Alternatively return an array of unused events so they won't be cleared.
   */
  public abstract setRealityEvents(gamificationEventsJSON: IRealityEventCacheCacheFile): boolean | IRealityEventCacheCacheFile | Promise<boolean | IRealityEventCacheCacheFile>;
  /**
